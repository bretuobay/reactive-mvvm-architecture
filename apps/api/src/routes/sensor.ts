...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
      return res
        .status(500)
        .json({ message: "Error creating sensor", error: error.message });
    }
...
router.get("/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
      return res
        .status(500)
        .json({ message: "Error fetching sensor", error: error.message });
    }
...
router.delete("/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
      return res
        .status(500)
        .json({ message: "Error deleting sensor", error: error.message });
    }
