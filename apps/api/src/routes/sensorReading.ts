...
const sensorReadingCreateSchema = z.object({
  sensorId: z.number({ required_error: "Sensor ID is required", invalid_type_error: "Sensor ID must be a number" }),
  timestamp: z.string({ required_error: "Timestamp is required" }).datetime({ message: "Timestamp must be a valid ISO 8601 date string" }),
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const newReading = await SensorReading.create({ sensorId, timestamp: new Date(timestamp), value });
    res.status(201).json(newReading);
  } catch (error) {
    console.error('Error creating sensor reading:', error);
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error creating sensor reading', error: error.message });
    }
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error fetching sensor readings', error: error.message });
    }
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error fetching sensor reading', error: error.message });
    }
...
router.get('/greenhouse/:greenhouseId', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error fetching readings for greenhouse', error: error.message });
    }
...
router.get('/sensor/:sensorId/history', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error fetching sensor history', error: error.message });
    }
