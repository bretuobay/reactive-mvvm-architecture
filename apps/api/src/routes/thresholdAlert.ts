...
const thresholdAlertBaseSchema = z.object({
  greenhouseId: z.number({ required_error: "Greenhouse ID is required", invalid_type_error: "Greenhouse ID must be a number" }),
  sensorType: z.enum(validSensorTypes, { required_error: "Sensor type is required" }),
  minValue: z.number({ required_error: "Minimum value is required", invalid_type_error: "Minimum value must be a number" }),
  maxValue: z.number({ required_error: "Maximum value is required", invalid_type_error: "Maximum value must be a number" }),
...
const thresholdAlertUpdateSchema = z.object({
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const existingAlert = await ThresholdAlert.findOne({ where: { greenhouseId, sensorType } });
    if (existingAlert) {
        return res.status(409).json({ message: `An alert for sensor type '${sensorType}' already exists for greenhouse ID ${greenhouseId}. You can update the existing one.` });
    }

    const newAlert = await ThresholdAlert.create({ greenhouseId, sensorType, minValue, maxValue });
    res.status(201).json(newAlert);
  } catch (error) {
    console.error('Error creating threshold alert:', error);
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error creating threshold alert', error: error.message });
    }
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error fetching threshold alerts', error: error.message });
    }
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error fetching threshold alert', error: error.message });
    }
  ...
  try {
    ...
    if (alert) {
      ...
      if (newMinValue >= newMaxValue) {
        return res.status(400).json({ message: 'minValue must be less than maxValue.', errors: { minValue: ['minValue must be less than maxValue.'] }});
      }
      ...
      if (updateData.greenhouseId !== undefined && updateData.greenhouseId !== alert.greenhouseId) {
        ...
        if (!greenhouse) {
          return res.status(404).json({ message: `Greenhouse with id ${updateData.greenhouseId} not found` });
        }
        // Check for conflict in new greenhouse
        if (updateData.sensorType && updateData.sensorType !== alert.sensorType || !updateData.sensorType) { // type changing or staying same
             const newSensorType = updateData.sensorType || alert.sensorType;
             const existingAlert = await ThresholdAlert.findOne({ where: { greenhouseId: updateData.greenhouseId, sensorType: newSensorType } });
             if (existingAlert) {
                 return res.status(409).json({ message: `An alert for sensor type '${newSensorType}' already exists for greenhouse ID ${updateData.greenhouseId}.` });
             }
        }
      }
      ...
      if (updateData.sensorType !== undefined && updateData.sensorType !== alert.sensorType) {
        const currentGreenhouseId = updateData.greenhouseId !== undefined ? updateData.greenhouseId : alert.greenhouseId;
        const existingAlert = await ThresholdAlert.findOne({ where: { greenhouseId: currentGreenhouseId, sensorType: updateData.sensorType } });
        if (existingAlert) {
          return res.status(409).json({ message: `An alert for sensor type '${updateData.sensorType}' already exists for greenhouse ID ${currentGreenhouseId}.` });
        }
      }
      
      await alert.update(updateData);
      const updatedAlertWithGH = await ThresholdAlert.findByPk(id, { include: [Greenhouse] });
    ...
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error updating threshold alert', error: error.message });
    }
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
        return res.status(500).json({ message: 'Error deleting threshold alert', error: error.message });
    }
