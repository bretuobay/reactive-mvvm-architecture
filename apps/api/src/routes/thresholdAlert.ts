...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const { greenhouseId, sensorType, minValue, maxValue } =
    ...
    if (!greenhouse) {
      return res
        .status(404)
        .json({ message: `Greenhouse with id ${greenhouseId} not found` });
    }
...
router.put("/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (alert) {
      ...
      if (
        updateData.greenhouseId !== undefined &&
        updateData.greenhouseId !== alert.greenhouseId
      ) {
        ...
        if (
          (updateData.sensorType &&
            updateData.sensorType !== alert.sensorType) ||
          !updateData.sensorType
        ) {
      ...
      if (
        updateData.sensorType !== undefined &&
        updateData.sensorType !== alert.sensorType
      ) {
        const currentGreenhouseId =
          updateData.greenhouseId !== undefined
            ? updateData.greenhouseId
            : alert.greenhouseId;
        const existingAlert = await ThresholdAlert.findOne({
          where: {
